...
export const generateSmartCriteria = createServerFn({ method: "POST" }).handler(
	async (ctx: { data: unknown }): Promise<SmartAssessment | null> => {
		// 1. Validate payload securely
		const { title, contextAnswers, overrides } = SmartRequestSchema.parse(
			ctx.data,
		);
...
Return ONLY a JSON object exactly matching this structure, with no markdown wrappers:
{
  ...
  "demultiplexer": { "requiresSplitting": boolean, "suggestedSplits": [], "suggestedPrerequisites": [] }
			...
			if (!response.ok) {
				...
				throw new Error(
					err.error?.message || "Failed to communicate with Gemini",
				);
			}
